<?php
    ...
    if(!empty($student_unique_id) &&!empty($lecturer_unique_id) && !empty($progress_stage) && !empty($proposal_due) && !empty($final_due)){
        ...
        if(mysqli_num_rows($select_query) > 0){
            ...
            $update_query1 = mysqli_query($conn, "UPDATE `student` SET `supervisor_unique_id`='".$lecturer_unique_id."' WHERE `unique_id`='".$student_unique_id."'");
           // $update_query2 = mysqli_query($conn, "UPDATE `progress` SET `email`='".$email."',`programme`='".$programme."',`year`='".$year."',`cgpa`='".$cgpa."',`phone_num`='".$phone_num."',`fyp_title`='".$fyp_title."',`supervisor_unique_id`='".$supervisor_unique_id."' WHERE `unique_id`='".$unique_id."'");
